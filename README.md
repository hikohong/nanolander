...
**Double-click a video in the tree and it does that for you.** One click previews, as any other file does; the second hands the file to `vim.ui.open` — `open` on macOS, `xdg-open` on a Linux desktop — so it goes to whatever owns the type. On a machine that has run [`bin/vlc-default`](#vlc-as-the-video-player) that is VLC. Nothing in the editor names VLC: the binding belongs to the system, and saying it in two places is how the two come apart. A directory still expands and every other file still opens in the editor pane, so the double click keeps doing what neo-tree documents. `<CR>` still previews, and a box with no desktop to hand a file to says so and points at `mpv --vo=tct` rather than going quiet. `lua/hikovim/video.lua` owns the one list of extensions both halves read, so a container cannot get a preview and no player.

...
| `editor_only_render_when_focused = false` | on, every return of focus to the terminal cleared each picture and rendered it again, and image.nvim answered with hundreds of renders in a quarter of a second — one alt-tab back measured at tens of megabytes of sixel, with the editor pane's picture cut in half on the way. A terminal keeps what it drew while it is in the background, so there is nothing to put back; tmux, where another window does draw over it, keeps its own switch |
...
**Moving through the tree previews without opening anything.** Land on a picture or a video — with `j` and `k`, or one click — and the pane above the tree swaps the outline for a thumbnail and the numbers; move onto anything else, or leave the tree, and the outline comes back. It is built for going fast: the thumbnail is a small copy, cached like every other, a burst of movement asks only once the cursor rests, and a thumbnail that finishes after the cursor has moved on is thrown away rather than drawn. **Choosing is what reaches the editor pane:** `<CR>` or a double click opens a picture there, drawn from its full copy, while a video, which the editor can only show one frame of, goes to the player. `gx` hands either one to the system instead. **The picture in the editor pane is left alone while you do.** image.nvim's sixel backend answers any change to any image by clearing the whole screen and repainting every image on it, so a thumbnail drawn through it repainted the editor's picture on every move — measured from the bytes the terminal receives: one or two full clears and a repaint of that picture per move, even onto a text file. The thumbnail paints its own sixel instead, and a move now sends that one small image and nothing else. It is taken off by writing spaces in the editor's background over exactly the rectangle it covered, and put back — once, however many renders asked — after image.nvim clears the screen for its own reasons, such as a resize.

**A thumbnail stays drawn once it lands.** It used to work for the first pass over a folder and then stop, which was a race hidden by the cache. Counting what reached the thumbnail's cells after it was sent turned up three ways Neovim wrote over it, and each is closed:

| What wrote over it | Why | Now |
| --- | --- | --- |
| `↵` and a clear-to-end-of-line, on every row | `~/.vimrc`'s `set list` draws `eol:↵` on an empty line, so the blank filler under a preview was never blank | preview windows get `nolist`, no number or sign columns and a blank `eob`, buffer-locally, so the next file in that window is unaffected |
| the redraw of those rows on every move | the buffer shrank to two lines while loading and grew back, so every row under the thumbnail changed twice | a fixed header over a body that is never rewritten |
| Neovim's own redraw, landing after the picture | the screen goes out through the TUI, a separate process, while a sixel goes straight to the terminal. Once a conversion was cached the sixel left in the same tick and the TUI's text arrived 0–1 ms behind it — which is why it only broke after the first pass | the redraw is flushed first and the sixel waits a moment behind it |

Measured over the same walk through a folder, before and after: a thumbnail intact at rest 3 times in 24, then 24 in 24; key to thumbnail 121 ms, then 87 ms, with `identify` remembered per file rather than asked on every move.
...
One detail is load-bearing rather than cosmetic: **the click that previews one of these leaves the cursor in the tree.** A mouse mapping is looked up in the buffer that is current when the key is processed, and the first click of a double click used to open the preview *and* jump to the editor pane — where `<2-LeftMouse>` is not mapped, so the second click reached nothing and the double click did nothing at all.
...
The thumbnail above the tree stopped appearing after a few moves, and the picture in the editor pane dragged and then vanished after switching around. Both were measured from the bytes a terminal receives, in a pty that answers like iTerm2. The thumbnail was being overwritten within a millisecond of landing — by the `↵` that `set list` draws on empty lines, and by a redraw the TUI delivered just after the sixel once conversions were cached. The editor's picture was collateral from focus: each return to the terminal set off hundreds of image.nvim renders, the thumbnail repainted once for every one of them — 3,365 sixels and 251 MB over seven alt-tabs — and the editor's 480 KB picture was cut apart in the crossfire. After: 18 sixels and 1.6 MB for the same script, the editor's picture intact at every rest, and a thumbnail intact at every rest on a picture.
